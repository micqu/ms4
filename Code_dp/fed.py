...
def run_experiment(experiment_name: str, args: TestParams, load_data_func: Callable, iteration=1):
	...
	central_model: Net = None
	if args.central_model_file != "":
		central_model = load_model(args.layer_sizes, args.central_model_file)
	...
	for it in range(iteration):
		...
		nodes = create_nodes(args.n_nodes, create_model, create_optimizer, args, device)
		...
		if args.n_nodes > 0:
			...
			for part_idx, data_idxs in data_map.items():
				...
				part_X, part_X_test, _, _ = model_selection.train_test_split(data_idxs, y_all[data_idxs], test_size=args.test_size, random_state=seed) #, stratify=y_all[data_idxs])
			...
			for i, node in enumerate(nodes):
				print(f"{i}", end=" ")			
				node.train_loader = get_data_loader(all_data, args.batch_size, kwargs, train_data_map[i])
				node.train_size = len(train_data_map[i])
				
				# Store local test loader on node for later ...
				node.test_loader = get_data_loader(all_data, args.test_batch_size, kwargs, test_data_map[i])
		...
		if args.modelType == ModelType.CENTRAL:
			...
			central_train_data_map = list(chain(*train_data_map.values()))
			# central_test_data_map = list(chain(*test_data_map.values()))
			train_loader = get_data_loader(all_data, args.batch_size, kwargs, central_train_data_map)
			# test_loader = get_data_loader(all_data, args.test_batch_size, kwargs, central_test_data_map)

			# Train metrics
			for epoch in range(args.epochs):
				print(f"\nEpoch {epoch}. Batch: ", end="")
				train_loss, epsilon, best_alpha = train_central(global_node, train_loader, args, device)
				...
				# Test on all clients with their own local data
				for node_idx, node in enumerate(nodes):
					train_metric_results = test_with_metrics(global_node.model, node.train_loader, "train_", args, device,
						acc=True, prec=True, rec=True, auc=True)
					print(f"Node {node_idx} model train metrics: {train_metric_results}\n")

					test_metric_results = test_with_metrics(global_node.model, node.test_loader, "test_", args, device,
			...
			for r in range(args.rounds):
				print(f"Round {r + 1}:")
				train_losses, epsilons, best_alphas = train(global_node, nodes, args, device)
				...
				for node_idx, node in enumerate(nodes):
					...
					node.metric_log.append([
						...
						test_metric_results.get("test_auc"),
						epsilons[node_idx],
						best_alphas[node_idx]
					])
				
				if r % 4 == 0:
					th.save(nodes[0].model.state_dict(), f"{experiment_path}/node_0_r{args.rounds}_s{seed}_{r}.pt")
