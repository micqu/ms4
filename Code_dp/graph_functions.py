	...
	for i, (path, legend) in enumerate(paths):
		...
		for key in keys:
			if plot_args.get("std") == True and std.get(key.value) is not None:
				# if sum(np.isnan(mean.get(key.value))) == 0 and sum(np.isnan(std.get(key.value))) == 0:
...
def get_model_predictions(layer_sizes, state_file):
...
def annotate_heatmap(im, data=None, valfmt="{x:.2f}",
					 ...
					 threshold=None, mask=None, **textkw):
	"""
